...
const imageStyle = (url, position='center') => {try{const parsed=new URL(String(url||''),window.location.href);if(!['http:','https:','blob:'].includes(parsed.protocol))return `background-position:${position}; background-size:cover; background-repeat:no-repeat;`;const safeUrl=parsed.href.replace(/["'()\\]/g,'');return `background-image:url("${safeUrl}"); background-position:${position}; background-size:cover; background-repeat:no-repeat;`}catch(error){return `background-position:${position}; background-size:cover; background-repeat:no-repeat;`}};
