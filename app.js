...
const state = { view: 'home', filter: 'all', query: '', selected: null, editing: null, user: null };
let listings = seed;
...
const imageStyle = url => `background-image:url('${url}')`;
...
async function loadSupabaseData(){
	...
	listings=(data || []).map(normalizeListing);if(!isAdmin())listings=listings.filter(item=>item.status!=='pending'&&item.status!=='rejected');render();
}
...
function openEditListingModal(item){
  ...
  form?.addEventListener('submit', async e => {
    ...
    const baseData = {title, type, price, location};
    ...
    const {error} = await supabaseClient.from('listings').update(updatePayload).eq('id', item.id).select().single();
    if(error){
      toast(error.message);
      return;
    }
...
document.addEventListener('click',event=>{if(event.target.closest('[data-nav="publish"]'))setTimeout(()=>{enhanceImageUpload();enhanceAreaField();enhancePriceField();enhanceListingFields()},0)});
...
document.addEventListener('submit',event=>{if(event.target.id!=='publish-form')return;event.preventDefault();event.stopImmediatePropagation();const form=event.target;const data=new FormData(form);const coverFiles=Array.from(form.querySelectorAll('input[name="coverImage"]')).flatMap(input=>Array.from(input.files||[])).filter(file=>file instanceof File&&file.size>0);const galleryFiles=Array.from(form.querySelectorAll('input[name="galleryImages"]')).flatMap(input=>Array.from(input.files||[])).filter(file=>file instanceof File&&file.size>0);const videoFiles=Array.from(form.querySelectorAll('input[name="videoFiles"]')).flatMap(input=>Array.from(input.files||[])).filter(file=>file instanceof File&&file.size>0);const type=data.get('type');const propertyType=data.get('propertyType');const area=Number(data.get('area'));const price=Number(String(data.get('price')||'').replace(/\D/g,''));const oversizedVideos=validateVideoFiles(videoFiles);if(!coverFiles.length && !galleryFiles.length && !videoFiles.length){toast('Selecciona al menos una foto o un video para la publicación.');return}if(galleryFiles.length>MAX_GALLERY_IMAGES){toast(`Máximo ${MAX_GALLERY_IMAGES} imágenes por publicación.`);return}if(videoFiles.length>MAX_VIDEO_COUNT){toast(`Máximo ${MAX_VIDEO_COUNT} videos por publicación.`);return}if(oversizedVideos.length){toast(`Los videos no pueden superar 50 MB. Reduce el archivo: ${oversizedVideos[0]}`);return}if(!price||price<=0){toast('Ingresa un precio válido.');return}if(type!=='vehicle'&&(!area||area<=0)){toast('Ingresa el área de la propiedad.');return}if(!supabaseClient||!state.user?.id){toast('Configura Supabase e inicia sesión para publicar.');return}Promise.all([Promise.all(coverFiles.map(file=>uploadMedia(file,'images'))),Promise.all(galleryFiles.map(file=>uploadMedia(file,'images'))),Promise.all(videoFiles.map(file=>uploadMedia(file,'videos')))]).then(async([coverUrls,galleryUrls,videoUrls])=>{const areaUnit=data.get('areaUnit')||'m²';const specs=type==='vehicle'?{brand:data.get('brand'),model:data.get('model'),year:data.get('year'),kms:data.get('kms'),fuel:data.get('fuel'),transmission:data.get('transmission')}:{propertyType,bedrooms:data.get('bedrooms'),bathrooms:data.get('bathrooms'),parking:data.get('parking'),access:data.get('access')};const description=`${data.get('desc')||'Publicación creada por la comunidad.'}${type==='vehicle'?'':`<!--area:${area}:${areaUnit}-->`}<!--specs:${encodeURIComponent(JSON.stringify(specs))}-->`;const combinedImages=[...coverUrls,...galleryUrls];const primaryImage=coverUrls[0]||combinedImages[0]||null;const status=isAdmin()?'approved':'pending';const item={type,category:type==='vehicle'?'Vehículo':propertyType,title:data.get('title'),price,location:data.get('location'),image:primaryImage,images:combinedImages.filter(Boolean),videos:videoUrls,desc:description,owner:state.user.email,status,...specs};const {data:created,error}=await supabaseClient.from('listings').insert({owner:state.user.id,type:item.type,category:item.category,title:item.title,price:item.price,location:item.location,image:item.image,images:item.images,videos:item.videos,description:item.desc,status}).select().single();if(error){toast(error.message);return}listings.unshift(normalizeListing(created));navigate('my');toast(isAdmin()?'Publicación creada y publicada':'Publicación enviada para aprobación')}).catch(error=>toast(error.message))},true);
...
document.addEventListener('click',event=>{if(event.target.closest('.profile-photo-button'))logoInput.click();setTimeout(applyLogo,0)});
